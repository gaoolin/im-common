...
public class WbOlpRawConsumer {
    ...
    private static final ThreadPoolSingleton singleThreadPool = ThreadPoolSingleton.getInstance();
    ...
    public void consume(List<ConsumerRecord<Long, WbOlpRawDataRecord>> records, Acknowledgment acknowledgment) {
        ...
        CompletableFuture<Void> allTasks = CompletableFuture.allOf(records.stream()
                .map(record -> CompletableFuture.runAsync(
                        ...
                        singleThreadPool.getTaskDispatcher().getExecutor(TaskPriority.IMPORTANT) // 使用虚拟线程
                )).toArray(CompletableFuture[]::new));

        singleThreadPool.getTaskDispatcher().dispatch(() -> {
            try {
