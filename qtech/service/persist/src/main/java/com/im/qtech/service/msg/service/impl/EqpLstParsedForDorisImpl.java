...
import com.im.qtech.service.msg.mapper.EqpLstParsedForDorisMapper;
...
@DS(DSName.SECOND)
@Service
public class EqpLstParsedForDorisImpl extends ServiceImpl<EqpLstParsedForDorisMapper, EqpLstParsedForDoris> implements IEqpLstParsedForDoris {

    private final SmartThreadPoolExecutor databaseExecutor;

    @Autowired
    public EqpLstParsedForDorisImpl(@Qualifier("importantTaskExecutor") SmartThreadPoolExecutor databaseExecutor) {
        // 使用自定义的线程池框架
        this.databaseExecutor = databaseExecutor;
    }

    @Override
    public CompletableFuture<Boolean> saveAsync(EqpLstParsedForDoris entity) {
        // 关键：在主线程中获取当前数据源设置
