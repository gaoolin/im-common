package com.im.inspection.dpp.data;

/**
 * author :  gaozhilin
 * email  :  gaoolin@gmail.com
 * date   :  2025/05/15 11:42:13
 * desc   :  一个更优雅、类型安全的方式。相比常量字符串类，枚举可以提供更好的封装性、可读性
 */


/**
 * 字段名称枚举类，用于统一管理 DataFrame 列名。
 */
public enum FieldEnum {


    // 通用字段
    SIM_ID("sim_id"),
    MC_ID("mc_id"),
    DT("dt"),
    FIRST_DRAW_TIME("first_draw_time"),
    LINE_NO("line_no"),
    PIECES_INDEX("pieces_index"),
    LEAD_X("lead_x"),
    LEAD_Y("lead_y"),
    PAD_X("pad_x"),
    PAD_Y("pad_y"),
    CHECK_PORT("check_port"),
    SUB_MC_ID("sub_mc_id"),
    CNT("cnt"),
    WIRE_LEN("wire_len"),
    CODE("code"),
    DESCRIPTION("description"),

    // fullLnMkSta 相关字段
    STD_LEAD_DIFF("std_lead_diff"),
    STD_PAD_DIFF("std_pad_diff"),
    LEAD_THRESHOLD("lead_threshold"),
    PAD_THRESHOLD("pad_threshold"),
    LEAD_OFFSET("lead_offset"),
    PAD_OFFSET("pad_offset"),

    // 其他中间字段
    LEAD_LEN("lead_len"),
    PAD_LEN("pad_len"),
    STD_MOD_LINE_CNT("std_mod_line_cnt"),
    CONFIRM_LABEL("confirm_label"),
    LINE_NO_ASC("line_no_asc"),
    LINE_NO_DESC("line_no_desc"),
    WIRE_LABEL("wire_label"),
    WIRE_LEN_TTL("wire_len_ttl"),
    WIRE_LEN_TTL_RANK("wire_len_ttl_rank"),
    LINE_NO_MOCK("line_no_mock");

    private final String fieldName;

    FieldEnum(String fieldName) {
        this.fieldName = fieldName;
    }

    public String get() {
        return fieldName;
    }

    @Override
    public String toString() {
        return fieldName;
    }
}

