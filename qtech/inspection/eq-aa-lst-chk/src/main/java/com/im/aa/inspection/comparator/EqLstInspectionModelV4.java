...
public class EqLstInspectionModelV4 extends AbstractParameterComparator<Object> {

    // 单例实例
    private static volatile EqLstInspectionModelV4 instance;

    // 私有构造函数
    private EqLstInspectionModelV4() {
    }

    // 获取单例实例
    public static EqLstInspectionModelV4 getInstance() {
        if (instance == null) {
            synchronized (EqLstInspectionModelV4.class) {
                if (instance == null) {
                    instance = new EqLstInspectionModelV4();
                }
            }
        }
        return instance;
