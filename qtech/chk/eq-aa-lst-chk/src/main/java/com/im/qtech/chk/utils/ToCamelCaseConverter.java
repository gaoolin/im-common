...
import com.im.qtech.chk.constants.EqLstCamelCaseConverter;
...
public class ToCamelCaseConverter {
    public static String doConvert(String input) {
        ...
        for (String keyword : EqLstCamelCaseConverter.ABBREVIATIONS) {
        ...
        // 步骤3：检查并处理首个字符和特殊字符
        if (!EqLstCamelCaseConverter.IGNORE_LIST_ITEMS.contains(input)) {
            if (!EqLstCamelCaseConverter.IGNORE_SPECIAL_CHARS.contains(input)) {
